...
describe('Game Mechanics - Playing Cards and Special Effects', () => {
  ...
  it('should allow drawing a card when player has no valid moves', async () => {
  ...
  it('should require Queen color selection', async () => {
  ...
  it('should update selected color when Queen color is selected', async () => {
  ...
  it('should handle card drawing from deck', async () => {
    return new Promise<void>((resolve) => {
      const service = interpret(gameMachine);
      const testCards = getTestCards();
      const players = [
  ...
  it('should detect win condition when player has no cards', async () => {
  ...
  it('should include draw penalty info in gameStatus after Queen color selection with active penalty', async () => {
  ...
  it('should handle Queen card selection', async () => {
  ...
  it('should not allow playing wrong color after Queen selection', async () => {
    // Test the validation function directly - this is a unit test for gameRules
    const { isValidMove } = await import('../utils/gameRules');
  ...
  it('should handle Ace played after Queen color selection and trigger skip effect', async () => {
    return new Promise<void>((resolve) => {
      const service = interpret(gameMachine);
      const testCards = getTestCards();
      const players = [
  ...
  it('should allow Ace of ANY color to be played after Queen color selection', async () => {
    // Verify basic game state is valid
    return new Promise<void>((resolve) => {
      const service = interpret(gameMachine);
      const testCards = getTestCards();
  ...
  it('should skip player without Ace when second Ace is played during stacking', async () => {
    /**
...
describe('Game Start Scenarios', () => {
  ...
  it('should deal 4 cards to each player at start', async () => {
    return new Promise<void>((resolve) => {
      const defaultPlayers = [
        { id: '1', name: 'Player1', color: 'blue', inHand: [] },
        { id: '2', name: 'Player2', color: 'red', inHand: [] },
  ...
  it('should have deck and game pile properly initialized', async () => {
    return new Promise<void>((resolve) => {
      const defaultPlayers = [
        { id: '1', name: 'Player1', color: 'blue', inHand: [] },
        { id: '2', name: 'Player2', color: 'red', inHand: [] },
  ...
  it('should update game status message on start', async () => {
    return new Promise<void>((resolve) => {
      const defaultPlayers = [
        { id: '1', name: 'Player1', color: 'blue', inHand: [] },
        { id: '2', name: 'Player2', color: 'red', inHand: [] },
...
describe('Draw Card While Can Play - Player Choice Scenario', () => {
  it('should allow player to draw card even when they have valid cards to play', async () => {
    /**
     * Scenario:
  ...
  it('should switch turn to next player after drawing a card', async () => {
    /**
     * Scenario:
    ...
    return new Promise<void>((resolve) => {
      ...
      let drawAttempted = false;
      let testComplete = false;
      let currentTurnBeforeDraw: string | null = null;
      let expectedNextTurn: string | null = null;
      
      service.onTransition((state) => {
        if ((state.value === 'playing' || typeof state.value === 'object') && !gameStarted) {
          gameStarted = true;
          currentTurnBeforeDraw = state.context.playerTurn;
          // Determine who should play next (the other player)
          expectedNextTurn = state.context.playerTurn === '1' ? '2' : '1';
          ...
          expect(state.context.playerTurn).not.toBe(currentTurnBeforeDraw);
          expect(state.context.playerTurn).toBe(expectedNextTurn);
  ...
  it('should allow drawing multiple times in sequence when players choose to draw', async () => {
    /**
     * Scenario:
